...
EOF
          ...
          kubectl -n jenkins delete pod kaniko-frontend-${TAG} --ignore-not-found
          kubectl -n jenkins apply -f /tmp/kaniko-fe.yaml
          for i in $(seq 1 120); do
            PH=$(kubectl -n jenkins get pod kaniko-frontend-${TAG} -o jsonpath='{.status.phase}' 2>/dev/null || echo "")
            if [ "$PH" = "Succeeded" ]; then echo "frontend 镜像 OK"; break; fi
            if [ "$PH" = "Failed" ]; then
              echo "frontend 构建失败，日志："
              kubectl -n jenkins logs kaniko-frontend-${TAG} --tail=200
              exit 1
            fi
            sleep 10
          done
          [ "$PH" = "Succeeded" ] || { echo "frontend 构建超时"; kubectl -n jenkins logs kaniko-frontend-${TAG} --tail=200; exit 1; }
        '''
      }
    }
